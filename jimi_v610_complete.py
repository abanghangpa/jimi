...
"""
╔═══════════════════════════════════════════════════════════════════════╗
║ JIMI FRAMEWORK v6.10 — Complete Self-Contained Package               ║
║ ETH/USDT 15m Multi-Module Scoring System + Liquidation Magnets       ║
║                                                                       ║
║ Modules: M1(1H MACD) M2(Multi-TF EMA) M3(VWAP+Vol+Taker)            ║
║          M4(15m CVD) M5(Liquidation Magnet)                          ║
║                                                                       ║
║ Usage:                                                                ║
║   python3 jimi_v610_complete.py backtest eth_15m_data.csv             ║
║   python3 jimi_v610_complete.py scan                                  ║
║   python3 jimi_v610_complete.py dashboard 8888                        ║
╚═══════════════════════════════════════════════════════════════════════╝
"""
...
def score_m3(df_15m, idx, direction):
    ...
    vol_ok = volume >= CONFIG['VOL_THRESHOLD'] * vol_avg20
    taker_ok = (taker_ratio >= CONFIG['TAKER_LONG']) if direction == 'LONG' else (taker_ratio <= CONFIG['TAKER_SHORT'])

    if in_zone and vol_ok and taker_ok:
        vwap_score = 1.0 - (vwap_dist / CONFIG['VWAP_ZONE_PCT'])
        vol_score = min(volume / (vol_avg20 * 2), 1.0)
        if direction == 'LONG':
            taker_score = min((taker_ratio - 0.5) / 0.1, 1.0)
        else:
            taker_score = min((0.5 - taker_ratio) / 0.1, 1.0)
        return 'PASS', vwap_score * 0.4 + vol_score * 0.3 + taker_score * 0.3, close
    return 'FAIL', 0.0, None
...
def run_backtest(csv_path, verbose=False, date_start=None, date_end=None):
    print("=" * 70)
    print("  JIMI FRAMEWORK v6.10 — Backtest Engine (5 Modules + Liquidation)")
    ...
    print("[4/6] Building timeframe index maps...")
    ...
    for idx in range(len(df_15m)):
        ...
        trade = Trade(ts, direction, entry_price, sl, tp1, tp2, tp3, size,
                      ...
                      ics, phase0_val,
                      f"M1={m1_dir} M2={m2_status} M3={m3_status} M4={m4_status} M5={m5_status} ICS={ics:.3f}")
        ...
        if verbose and stats['entries'] <= 50:
            print(f"  ENTRY #{stats['entries']}: {ts} {direction} @ {entry_price:.2f} "
                  f"SL={sl:.2f} TP1={tp1:.2f} ICS={ics:.3f} M5={m5_status}({m5_score:.2f}) size={size:.2f}")
...
def print_report(trades, stats):
    ...
    print(f"\n  Signal Flow:")
    for k in ['signals_checked','m1_neutral_skip','m3_fail','m2_neutral_long_skip','long_disabled','long_ics_skip','long_m5_skip','long_phase0_skip','ics_blocked','ics_ceiling_skip','m4_required_skip','m4_false_anchored','m5_pass','m5_fail','cascade_detected','dedup_skip','filter_blocked','consec_pause','entries']:
        if k in stats:
...
.mod-score.PASS{color:#00ff88}.mod-score.FAIL{color:#ff4444}.mod-score.NEUTRAL{color:#ff8800}
...
.refresh{color:#444;font-size:11px}</style></head><body>
<h1>⚡ JIMI v6.10</h1><div class="subtitle">ETH/USDT 15m — 5 Module Signal Scanner + Liquidation Magnets</div>
<div class="grid">
...
let mods='';[['M1','m1'],['M2','m2'],['M3','m3'],['M4','m4'],['M5','m5']].forEach(([n,k])=>{const m=d[k];if(!m)return;
