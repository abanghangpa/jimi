#!/usr/bin/env python3
"""Fetch March ETH/USDT 15m data for each year and run JIMI backtests."""
...
for year in [2020, 2021, 2022, 2023, 2024, 2025]:
    start = pd.Timestamp(f'{year}-01-15', tz='UTC')
    end = pd.Timestamp(f'{year}-04-01', tz='UTC')
    ...
    print(f"  Running backtest for March {year}...")
    try:
        trades, stats, df_bt = run_backtest(
            csv_path, verbose=False,
            date_start=f'{year}-03-01', date_end=f'{year}-03-31',
        )
...
if valid:
    ...
    print('-'*80)
    print(f"  Total trades across all Marchs: {total_trades}")
    print(f"  Aggregate win rate: {avg_wr:.1f}% ({total_wins}W / {total_losses}L)")
    print(f"  Avg March PnL: {avg_pnl:+.2f}%")
    print(f"  Best March: {best['year']} ({best['total_pnl']:+.2f}%)")
    print(f"  Worst March: {worst['year']} ({worst['total_pnl']:+.2f}%)")
    profitable = len([r for r in valid if r['total_pnl'] > 0])
    print(f"  Profitable Marchs: {profitable}/{len(valid)}")
