#!/usr/bin/env python3
"""Fetch December ETH/USDT 15m data for each year and run JIMI backtests."""
...
for year in [2020, 2021, 2022, 2023, 2024, 2025]:
    # Fetch Oct 15 of (year) to Jan 1 of (year+1) for warmup + December
    start = pd.Timestamp(f'{year}-10-15', tz='UTC')
    end = pd.Timestamp(f'{year+1}-01-01', tz='UTC')
    since_ms = int(start.timestamp() * 1000)
    until_ms = int(end.timestamp() * 1000)

    print(f"\n{'='*60}")
    print(f"  Fetching data for December {year} (Oct 15 - Dec 31)...")
    print(f"{'='*60}")

    try:
        df = fetch_ohlcv('ETH/USDT', '15m', since_ms, until_ms)
    except Exception as e:
        print(f"  ERROR fetching Dec {year}: {e}")
        results.append({'year': year, 'error': str(e)})
        continue

    if df.empty:
        print(f"  No data for Dec {year}")
        results.append({'year': year, 'error': 'no data'})
        continue

    print(f"  Fetched {len(df)} bars ({df['Open time'].iloc[0]} to {df['Open time'].iloc[-1]})")

    csv_path = f'/tmp/eth_15m_{year}_dec.csv'
    df.to_csv(csv_path, index=False)

    print(f"  Running backtest for December {year}...")
    try:
        trades, stats, df_bt = run_backtest(
            csv_path, verbose=False,
            date_start=f'{year}-12-01', date_end=f'{year}-12-31',
        )
...
# ── Summary ──
print(f"\n\n{'='*80}")
print(f"  JIMI v6.10 — DECEMBER BACKTEST SUMMARY (ETH/USDT 15m)")
print(f"{'='*80}")
...
if valid:
    ...
    print('-'*80)
    print(f"  Total trades across all Decembers: {total_trades}")
    print(f"  Aggregate win rate: {avg_wr:.1f}% ({total_wins}W / {total_losses}L)")
    print(f"  Avg December PnL: {avg_pnl:+.2f}%")
    print(f"  Best December: {best['year']} ({best['total_pnl']:+.2f}%)")
    print(f"  Worst December: {worst['year']} ({worst['total_pnl']:+.2f}%)")
    profitable = len([r for r in valid if r['total_pnl'] > 0])
    print(f"  Profitable Decembers: {profitable}/{len(valid)}")
