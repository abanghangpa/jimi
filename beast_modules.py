...
BEAST_CONFIG = {
    ...
    'DATA_FRESHNESS_MAX_AGE_MIN': 20,      # max staleness before forcing flat
    'DATA_FRESHNESS_CHECK_INTERVAL': 5,    # check every N bars in backtest

    # --- Veto Priority System ---
    'VETO_ENABLED': True,
    'VETO_CRISIS_HARD': True,              # M9 CRISIS = hard block regardless of ICS
    'VETO_CHOP_HARD': False,               # M9 CHOP = soft (size reduction only)
    'VETO_STALE_DATA_HARD': True,          # stale derivatives = hard block
    'VETO_MONTHLY_DD_HARD': True,          # monthly DD = hard block
    'VETO_DIR_CONFLICT_HARD': True,        # M4+M5 both against = hard block
}
...
# ═══════════════════════════════════════════════════════════════════════
#
# Detects stale/missing data to prevent trading on "zombie" values.
# If derivatives data is older than threshold, force flat / block entry.
...
def check_data_freshness(df_deriv=None, max_age_minutes=20, current_time=None):
    ...
    Returns: (is_fresh: bool, age_minutes: float, details: dict)
    """
    details = {}
    ...
    # Check for data gaps (consecutive identical timestamps = ffill zombie)
    if len(df_deriv) >= 3:
...
#   HARD VETO  — Kill signal immediately, regardless of ICS or other modules
#   SOFT VETO  — Reduce confidence, but allow if ICS is strong enough
#   WEIGHTED   — Normal ICS scoring
#
# Hard vetoes override everything. A single hard veto = no trade.

class VetoResult:
    """Result of veto evaluation."""
    def __init__(self):
        self.allowed = True
        self.hard_vetoes = []      # list of (module, reason)
        self.soft_vetoes = []      # list of (module, reason, penalty)
        self.warnings = []         # non-blocking alerts
...
def evaluate_vetoes(
    ...
    Priority order (checked in sequence, first hard veto wins):
      1. Data Freshness — stale data = hard block
      2. Vol Regime — CRISIS = hard block, CHOP = soft/hard (configurable)
      3. Monthly DD — circuit breaker hit = hard block
      4. Direction Conflict — M4+M5 both against = hard block
      5. Module failures — M10/M11 failures = soft vetoes
    """
    result = VetoResult()
