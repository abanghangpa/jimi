#!/usr/bin/env python3
"""Fetch July ETH/USDT 15m data for each year and run JIMI backtests."""
...
for year in [2020, 2021, 2022, 2023, 2024, 2025]:
    start = pd.Timestamp(f'{year}-05-15', tz='UTC')
    end = pd.Timestamp(f'{year}-08-01', tz='UTC')
    ...
    print(f"  Running backtest for July {year}...")
    try:
        trades, stats, df_bt = run_backtest(
            csv_path, verbose=False,
            date_start=f'{year}-07-01', date_end=f'{year}-07-31',
        )
...
if valid:
    ...
    print('-'*80)
    print(f"  Total trades across all Julys: {total_trades}")
    print(f"  Aggregate win rate: {avg_wr:.1f}% ({total_wins}W / {total_losses}L)")
    print(f"  Avg July PnL: {avg_pnl:+.2f}%")
    print(f"  Best July: {best['year']} ({best['total_pnl']:+.2f}%)")
    print(f"  Worst July: {worst['year']} ({worst['total_pnl']:+.2f}%)")
    profitable = len([r for r in valid if r['total_pnl'] > 0])
    print(f"  Profitable Julys: {profitable}/{len(valid)}")
