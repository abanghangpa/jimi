...
CONFIG = {
    ...
    # --- ADAPTIVE WEIGHTS ---
    "ADAPTIVE_WEIGHTS_ENABLED": True,   # enable adaptive module weighting
    "ADAPTIVE_DECAY": 0.97,             # exponential decay factor (0.97 = slower adaptation)
    "ADAPTIVE_MIN_MULT": 0.8,           # minimum multiplier for a module weight
    "ADAPTIVE_MAX_MULT": 1.2,           # maximum multiplier for a module weight
    "ADAPTIVE_WARMUP_TRADES": 15,       # minimum trades before adaptive kicks in

    # --- SESSION AWARENESS ---
    "SESSION_AWARENESS_ENABLED": True,
    "SESSION_ASIAN_MULT": 0.85,         # reduce confidence 15% in Asian session (00-08 UTC)
    "SESSION_EU_MULT": 1.0,             # EU session (08-14 UTC) — neutral
    "SESSION_US_MULT": 1.05,            # US session (14-22 UTC) — boost 5%
    "SESSION_LATE_US_MULT": 0.90,       # Late US (22-00 UTC) — reduce 10%
    "SESSION_US_OPEN_BOOST": 1.10,      # extra boost at US open (14-16 UTC)

    # --- M8: FUNDING RATE MODULE ---
    "M8_ENABLED": True,
    "M8_WEIGHT": 0.10,                  # funding rate weight in ICS
    "M8_HIGH_FUNDING": 0.05,            # funding > 0.05% → shorts favored
    "M8_LOW_FUNDING": -0.05,            # funding < -0.05% → longs favored
    "M8_FLIP_BONUS": 0.15,              # extra ICS boost on funding flip

    # --- MULTI-TF ENTRY CONFIRMATION ---
    "MTF_CONFIRM_ENABLED": True,
    "MTF_1H_CANDLE_CHECK": True,        # require 1H candle to agree with direction
    "MTF_4H_EMA_CHECK": False,           # disabled by default — too aggressive

    # --- M1 ENHANCEMENT ---
    "M1_RSI_ENABLED": True,             # add RSI divergence to M1
    "M1_MOMENTUM_ENABLED": True,        # add momentum shift detection
    "M1_RSI_PERIOD": 14,
    "M1_RSI_OVERBOUGHT": 70,
    "M1_RSI_OVERSOLD": 30,
    "M1_MOMENTUM_LOOKBACK": 6,          # bars for momentum calculation

    # --- CROSS-ASSET CORRELATION ---
    "CROSS_ASSET_ENABLED": True,
    "CROSS_ASSET_BTC_WEIGHT": 0.08,     # BTC correlation weight
    "CROSS_ASSET_LOOKBACK": 48,         # bars for correlation calc (48 × 15m = 12h)
}
...
# ─── M8: FUNDING RATE MODULE ──────────────────────────────────────
def score_m8_funding(funding_rate, direction):
    """Score funding rate as directional signal.

    High positive funding → longs paying shorts → shorts have edge
    High negative funding → shorts paying longs → longs have edge
    """
    ...
    if funding_rate > high:
        score = min(0.3 + (funding_rate - high) * 5, 0.2)  # lower score = bearish
    ...
    # Funding favors longs
        ...
        score = min(0.3 + abs(funding_rate - low) * 5, 0.2)  # lower score = bullish actually means high for longs
        agrees = direction == 'LONG'
        details['bias'] = 'LONGS_FAVORED'
        # For longs, we want HIGH score; for shorts, LOW score
        if direction == 'LONG':
...
def score_cross_asset(eth_price, btc_price, btc_corr, btc_change_1h, direction):
    ...
    - BTC dropping hard + high correlation → ETH likely follows → SHORT bias
    - BTC rallying + high correlation → ETH likely follows → LONG bias
    - Low correlation → cross-asset signal is weak
    """
    if btc_price is None or np.isnan(btc_corr):
        return 0.5  # neutral

    # If correlation is low, cross-asset signal is weak
    if abs(btc_corr) < 0.3:
        return 0.5

    # BTC direction vs ETH direction alignment
    btc_bullish = btc_change_1h > 0.005   # BTC up > 0.5%
    btc_bearish = btc_change_1h < -0.005  # BTC down > 0.5%

    if direction == 'LONG':
        if btc_bullish and btc_corr > 0.5:
            return 0.7  # BTC confirms long
        elif btc_bearish and btc_corr > 0.5:
            return 0.3  # BTC contradicts long
    elif direction == 'SHORT':
        if btc_bearish and btc_corr > 0.5:
            return 0.7  # BTC confirms short
        elif btc_bullish and btc_corr > 0.5:
            return 0.3  # BTC contradicts short

    return 0.5

# ─── ENHANCED M1: MACD + RSI DIVERGENCE + MOMENTUM ────────────────
def score_m1(df_1h, idx):
    """Enhanced M1: MACD histogram + RSI divergence + momentum shift.

    Combines three signals:
    1. MACD histogram direction (original)
    2. RSI divergence (price vs RSI disagreement)
    3. Momentum acceleration/deceleration
    """
    if idx < 2:
        return 'NEUTRAL', 0.5

    # --- Signal 1: MACD Histogram (original) ---
    ...
    rsi_score = None  # None = not active, don't dilute MACD
    if CONFIG.get('M1_RSI_ENABLED', False) and 'rsi' in df_1h.columns:
        ...
        # Bearish divergence: price up but RSI down (weakening momentum)
        if close > close_prev and rsi < rsi_prev and rsi > 55:
            rsi_score = 0.35  # bearish signal
        # Bullish divergence: price down but RSI up (strengthening momentum)
        elif close < close_prev and rsi > rsi_prev and rsi < 45:
            rsi_score = 0.65  # bullish signal
        # RSI extreme overbought — reversal risk
        elif rsi > ob:
            rsi_score = 0.4
        # RSI extreme oversold — bounce risk
        elif rsi < os_:
            rsi_score = 0.6

    # --- Signal 3: Momentum Shift ---
    mom_score = None  # None = not active
    if CONFIG.get('M1_MOMENTUM_ENABLED', False):
        ...
        if idx >= lookback:
            ...
            if roc_now > 0.02 and accel > 0:
                mom_score = 0.8  # strong bullish momentum accelerating
            elif roc_now > 0.02 and accel < 0:
                mom_score = 0.6  # bullish but decelerating
            elif roc_now < -0.02 and accel < 0:
                mom_score = 0.2  # strong bearish momentum accelerating
            elif roc_now < -0.02 and accel > 0:
                mom_score = 0.4  # bearish but decelerating
    ...
    # Only add signals that are actually active (not neutral-diluting)
    ...
    if combined > 0.6:
        ...
        direction = macd_dir  # fall back to MACD direction
...
# ═══════════════════════════════════════════════════════════════════════
# ADAPTIVE MODULE WEIGHTING (Position Sizing)
# ═══════════════════════════════════════════════════════════════════════

class AdaptiveWeights:
    """Track per-module accuracy and adjust POSITION SIZE (not ICS).

    Instead of changing which trades are taken, this system keeps trade
    selection identical and adjusts how much capital to risk:

    - Modules performing well → their agreement with a trade boosts size
    - Modules performing poorly → their agreement matters less

    This avoids the feedback loop where adaptive ICS weights change trade
    selection, which changes what the weights learn, which changes selection...

    Size multiplier range: [0.7, 1.3] — subtle but meaningful over many trades.
    """
    ...
    def get_multipliers(self):
        """Return adaptive weight multipliers for each module."""
    ...
    def size_multiplier(self, direction, m1_dir, m2_status, m3_score, m4_status, m5_status):
        """Compute position size multiplier based on module agreement.

        For each module that agrees with the trade direction AND has high
        accuracy, boost size. For modules that disagree AND have high accuracy,
        reduce size.
        """
        if self.trade_count < self.warmup:
            return 1.0
    ...
    def summary(self):
        """Print adaptive weight diagnostics."""
        if self.trade_count < self.warmup:
...
def calc_ics(m1_score, m2_score, m3_score, m4_score, m4_status, m5_score=0.5, m6_score=0.5, m7_score=0.5, m8_score=0.5, cross_asset_score=0.5, use_derivatives=False, use_m7=False, use_m8=False, use_cross_asset=False, cascade_dir='NONE', cascade_strength=0.0):
    m4_contrib = m4_score if m4_status == 'PASS' else 0.5

    # Determine how many extra modules are active
    extra_modules = []
    ...
    if extra_modules:
        # Redistribute weights to include extra modules
...
def run_backtest(csv_path, verbose=False, date_start=None, date_end=None):
    ...
    # --- Enhanced M1: RSI on 1H ---
    if CONFIG.get('M1_RSI_ENABLED', False):
    ...
    # --- Cross-Asset: Fetch BTC 15m data ---
    btc_15m_df = None
    btc_corr_series = None
    if CONFIG.get('CROSS_ASSET_ENABLED', False):
        ...
        try:
            ...
            if btc_15m_df is not None and len(btc_15m_df) > 100:
                btc_corr_series = compute_btc_correlation(df_15m, btc_15m_df, CONFIG.get('CROSS_ASSET_LOOKBACK', 48))
    ...
    if CONFIG.get('M8_ENABLED', False):
        try:
    ...
    stats = {
        ...
        'mtf_blocked': 0, 'm8_pass': 0, 'm8_fail': 0,
    }
    ...
    if CONFIG.get('ADAPTIVE_WEIGHTS_ENABLED', False):
        adaptive_tracker = AdaptiveWeights(
            decay=CONFIG.get('ADAPTIVE_DECAY', 0.95),
            min_mult=CONFIG.get('ADAPTIVE_MIN_MULT', 0.3),
            max_mult=CONFIG.get('ADAPTIVE_MAX_MULT', 2.0),
            warmup=CONFIG.get('ADAPTIVE_WARMUP_TRADES', 10),
        )
    ...
    for idx in range(len(df_15m)):
        ...
        # --- Update adaptive weights for newly closed trades ---
        ...
        if CONFIG.get('CROSS_ASSET_ENABLED', False) and btc_15m_df is not None and btc_corr_series is not None:
        ...
        if CONFIG.get('MTF_CONFIRM_ENABLED', False):
            mtf_block = False

            # Check 1: 1H candle direction matches trade direction
            if CONFIG.get('MTF_1H_CANDLE_CHECK', False):
                if idx_1h >= 0:
            ...
            # Check 2: 4H EMA alignment
            if CONFIG.get('MTF_4H_EMA_CHECK', False) and not mtf_block:
                if idx_4h >= 0:
            ...
            if mtf_block:
                stats['mtf_blocked'] = stats.get('mtf_blocked', 0) + 1
                continue
    ...
    if open_trades:
        ...
        # Update adaptive tracker for end-of-backtest closes
        if adaptive_tracker is not None:
            for t in open_trades:
                if not t.is_open:
                    adaptive_tracker.update(t)
