        ...
        'cascade': [
            {
                ...
                'action': 'First mover. Contrary to intuition: STRONG BEAT is bearish (-0.55% avg, 29% WR). Mild beat is neutral. Miss is bearish. "Sell the news" on strong prints.',
                'duration': '~6h',
                'key_signal': 'Direction sets tone. Strong beat = fade, mild beat = hold, miss = sell.',
            },
            {
                'session': 'EUROPE (same day)',
                'action': 'London inherits Asia direction 93% of the time. Front-runs upcoming Eurozone Flash PMI. Strong continuation signal.',
                'duration': '~6h',
                'key_signal': 'Europe continuation of Asia = high conviction. Europe reversal = rare, watch for.',
            },
            {
                'session': 'US (same day)',
                'action': 'New York calibrates baseline for US ISM Manufacturing PMI (~2 days later). US amplifies or fades the move. Best session for BEAT signals (+0.97% avg, 60% WR).',
                'duration': '~8h',
                'key_signal': 'US session is where BEAT pays off. STRONG_BEAT continues dumping.',
            },
            {
                'session': 'ASIA RE-OPEN (next day)',
                'action': 'NBS PMI drops. Caixin/NBS divergence does NOT reliably cause reversal (only 15% rate). Trend continuation more likely than reversal.',
                'duration': '~8h',
                'key_signal': 'NBS divergence = weak reversal signal. Trend continuation dominates.',
            },
        ],
        'trading_implication': 'STRONG BEAT = SELL (sell-the-news, -1.9% avg 24h). BEAT = BUY (+1.9% avg 24h). INLINE = best signal (+2.5% avg). Session inheritance is 93% — Europe follows Asia.',
        'backtest_stats': {
            'events': 29,
            'strong_beat_24h': -1.90,
            'beat_24h': +1.92,
            'inline_24h': +2.51,
            'miss_24h': -2.71,
            'session_inheritance_rate': 0.93,
            'nbs_divergence_reversal_rate': 0.15,
        },
    },
    'cn_nbs_pmi': {
        'release_session': 'asia',
